...
const DEFAULT_ZOOM_HEIGHT = 60;
const MIN_ZOOM_HEIGHT = 20;
const MAX_ZOOM_HEIGHT = 140;
const ZOOM_STEP = 10;
...
function initMapView({ camera, scene, player, renderer }) {
...
function update(dt) {
  ...
  if (!pose) return;
  const followAlpha = 1 - Math.exp(-dt * 6);
  state.camera.position.lerp(pose.position, followAlpha);
  state.camera.quaternion.slerp(pose.quaternion, followAlpha);
  state.camera.up.copy(state.mapUp);
}
